...
(define lecture-classifications
  (for/list ([i (in-range 1 6)]) (number->string i)))
(define activity-classifications
  (for/list ([i (in-range 6 15)]) (number->string i)))
...
(define (compute-wtus course)
  (define enrollment (string->number (col-ref/g 'enrollment course)))
...
(define (has-group? course)
  (not (equal? (col-ref/g 'group-code course) "")))

(define (is-senior-project? course)
  (member (list (col-ref/g 'dept course)
